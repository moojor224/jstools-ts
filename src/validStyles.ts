...
export const validStyles = (function () {
    try {
        ...
        function* run() {
            // @ts-ignore
            const style: CSSStyleDeclaration = getComputedStyle(document.createElement("div"));
            for (const i in style) {
                if (typeof style[i] === "string" && !i.startsWith("css")) {
                    yield i;
                }
            }
        };
        const s = [...run()];
        // console.info("Using CSS properties from browser");
        return s;
    } catch {
        ...
        try {
            ...
            for (const e of other.concat(other2)) {
                keys.add(e);
            }
            // console.info("Using CSS properties from w3.org");
            return Array.from(keys);
        } catch { }
    }
    // fallback to hardcoded properties
    // console.info("Using hardcoded CSS properties");
    return [
